Describe 'The firewall' {
    ...
    Context 'should allow syncthing' {
        It 'on port 8384'{
            ($ufwOutput | Where-Object {$_ -match '(8384/tcp)\s*(ALLOW)\s*(Anywhere)'} ) | Should Not Be $null
        }

        It 'on port 20000'{
            ($ufwOutput | Where-Object {$_ -match '(20000/tcp)\s*(ALLOW)\s*(Anywhere)'} ) | Should Not Be $null
        }

        It 'on port 21027' {
            ($ufwOutput | Where-Object {$_ -match '(21027/tcp)\s*(ALLOW)\s*(Anywhere)'} ) | Should Not Be $null
        }
    }
